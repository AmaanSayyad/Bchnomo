                        ...
                        `}>
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black uppercase tracking-tighter leading-none opacity-60">
                                    {(lastResult.currency || (network === 'ARB' ? 'ETH' : network === 'SOL' ? 'SOL' : network === 'SUI' ? 'USDC' : network === 'XLM' ? 'XLM' : network === 'XTZ' ? 'XTZ' : network === 'NEAR' ? 'NEAR' : 'BNB'))} {lastResult.won ? 'PROFIT' : 'LOSS'}
                                </span>
                                <span className="text-sm font-mono font-black tracking-tight leading-tight">
                                    {lastResult.won ? '+' : '-'}{Math.abs(lastResult.won ? lastResult.payout : lastResult.amount).toFixed(4)} <span className="text-[10px] opacity-70">{(lastResult.currency || (network === 'ARB' ? 'ETH' : network === 'SOL' ? 'SOL' : network === 'SUI' ? 'USDC' : network === 'XLM' ? 'XLM' : network === 'XTZ' ? 'XTZ' : network === 'NEAR' ? 'NEAR' : 'BNB'))}</span>
                                </span>
                            </div>
