'use client';
// Force rebuild - revert to original design

import React from 'react';
import { useOverflowStore } from '@/lib/store';
import { reownModal } from 'bch-connect';
...
export const WalletConnectModal: React.FC = () => {
    ...
    const handleNativeBCHConnect = async () => {
        try {
            setOpen(false);
            setPreferredNetwork('BCH');
            // This will open the WalletConnect QR modal
            reownModal.open();
        } catch (error) {
            console.error("Native BCH WalletConnect error:", error);
        }
    };
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                >
                    ...
                    <div className="p-4 sm:p-6 space-y-2 sm:space-y-3 overflow-y-auto no-scrollbar">
                        {/* Native BCH WalletConnect Option (ONLY ONE) */}
                        <button
                            onClick={handleNativeBCHConnect}
                        ...
                        >
                            ...
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-green-500/10 flex items-center justify-center border border-green-500/20 group-hover:scale-110 transition-transform shrink-0">
                                <img src="/logos/bitcoin-cash-bch-logo.png" alt="BCH" className="w-6 h-6 sm:w-7 sm:h-7" />
                            </div>
                            <div className="flex-1 text-left">
                                <div className="flex items-center gap-2">
                                    <span className="font-bold text-white text-sm sm:text-base">Trust Wallet / Mobile</span>
                                    <span className="px-1.5 py-0.5 rounded text-[8px] sm:text-[10px] bg-green-500/20 text-green-500 font-bold uppercase tracking-wider">Native BCH</span>
                                </div>
                                <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5">WalletConnect v2 (QR Code)</p>
                            </div>
                            <Globe className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600 group-hover:text-green-500 transition-colors" />
                        </button>
                    </div>
                    ...
                    <div className="p-4 bg-white/5 text-center shrink-0">
                        <p className="text-[8px] sm:text-[10px] text-gray-500 uppercase tracking-widest font-bold">
                            Bchnomo Protocol · Pyth Hermes
                        </p>
                        <p className="text-[9px] text-gray-600 mt-1">Powered by Pyth Hermes · Bchnomo Protocol</p>
                    </div>
                </motion.div>
            </div>
        </AnimatePresence>
